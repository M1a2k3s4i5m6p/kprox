...
// ---- HTTPS GET helper ----
// Uses WiFiClientSecure with certificate verification disabled — the device
// has no root CA store, and GitHub's cert chain changes regularly. The
// connection is still TLS-encrypted; we're just not validating the cert.
...
// ---- Fetch gadgets from GitHub ----

void AppGadgets::_fetch() {
    _state       = ST_LOADING;
    _gadgets.clear();
    _installMsg  = "";
    _needsRedraw = true;
    _draw();   // show loading immediately before blocking fetch

    // 1. List files in /gadgets directory
    ...
    // Parse file array from GitHub contents API
    JsonDocument dir;
    ...
    for (JsonVariant entry : dir.as<JsonArray>()) {
        ...
        JsonDocument gdoc;
        if (deserializeJson(gdoc, raw)) continue;
...
// ---- Install gadget to new register ----
...
void AppGadgets::_drawLoading() {
    ...
    disp.drawString("Fetching from GitHub...", 4, BAR_H + 16);
    disp.drawString("(requires WiFi)", 4, BAR_H + 32);
}
...
void AppGadgets::_drawError() {
    ...
    // Word-wrap error message at ~34 chars
...
void AppGadgets::_drawGadget() {
    ...
    // Description — up to 3 lines of ~34 chars
    disp.setTextColor(disp.color565(180, 220, 255), GDG_BG);
    ...
    // Content preview (first ~55 chars)
